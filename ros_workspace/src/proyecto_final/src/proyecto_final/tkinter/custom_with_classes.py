...
class DynamicTabsApp:
    ...
    def create_notebook(self):
        ...
        self.tab_vision = ttk.Frame(self.notebook, relief='flat')
        self.tab_vision.grid(row=0, column=0, sticky="nsew")
        ...
        # --- NOTEBOOK FRAME: Robótica ---
        self.tab_robotics = ttk.Frame(self.notebook, relief='flat')
        self.tab_robotics.grid(row=0, column=0, sticky="nsew")
        self.tab_robotics.grid_columnconfigure(0, weight=1)
        self.tab_robotics.grid_rowconfigure(0, weight=1)
        self.notebook.add(self.tab_robotics, text=self.tabs[1])
        self.RoboticaTab = RoboticaTab(self.root, self.tab_robotics, False)
        
        # --- NOTEBOOK FRAME: Reinforcement Learning ---
    ...
    def on_tab_changed(self, event):
        ...
        if active_tab == str(self.tab_vision):
            if self.prev_active_tab == str(self.tab_robotics):
                self.RoboticaTab.stop_robot_tab()
            self.VisionTab.tab_active = True
            self.RoboticaTab.tab_active = False
            self.VisionTab.camera_controller.start(10)
            ...
            self.RoboticaTab.start_robot_tab()
            ...
            self.VisionTab.tab_active = False
            self.RoboticaTab.tab_active = False
        self.prev_active_tab = active_tab
